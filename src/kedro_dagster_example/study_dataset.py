...
class StudyDataset(AbstractVersionedDataset[optuna.Study, optuna.Study]):
    ...
    def _validate_backend(self, backend):
        ...
        if backend not in valid_backends:
            raise ValueError(f"Requested `backend` '{backend}' is not registered as an SQLAlchemy dialect.")
        return backend
    ...
    def _get_save_study_name(self) -> str:
        ...
        if self._exists_function(str(versioned_study_name)):
            raise DatasetError(
                f"Study name '{versioned_study_name}' for {self!s} must not exist if versioning is enabled."
            )
    ...
    def _get_sampler(self, sampler_config):
        ...
        if sampler_class_name in ["QMCSampler", "CmaEsSampler", "GPSampler"]:
            sampler_config["independent_sampler"] = self._get_sampler(sampler_config.pop("independent_sampler"))

        if sampler_class_name == "PartialFixedSampler":
            sampler_config["base_sampler"] = self._get_sampler(sampler_config.pop("base_sampler"))
    ...
    def _get_pruner(self, pruner_config):
        ...
        if pruner_class_name == "PatientPruner":
            pruner_config["wrapped_pruner"] = self._get_pruner(pruner_config.pop("wrapped_pruner"))
